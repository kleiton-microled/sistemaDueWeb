...
{
    public partial class ConsultaEstoquePreACD_New2 : System.Web.UI.Page
    {
        ...
        {
            LogsService.Logar("ConsultaEstoquePreACD_NEW2.aspx", $"Iniciou consulta no Banco as {DateTime.Now.ToString()}");
            var dataTable = _estoqueDAO.GerarCsvEstoque(GerarFiltro());
            LogsService.Logar("ConsultaEstoquePreACD_NEW2.aspx", $"Finalizou consulta no Banco as {DateTime.Now.ToString()}");
            var agora = DateTime.Now;
            var nomeArquivo = Path.Combine(Server.MapPath("."), "Uploads", $"LM_{agora.ToString("ddMMyyyy")}_{agora.ToString("HHmmss")}.csv");

            LogsService.Logar("ConsultaEstoquePreACD_NEW2.aspx", $"Começou montar o arquivo as {DateTime.Now.ToString()}");
            var sw = new StreamWriter(new FileStream(nomeArquivo, FileMode.CreateNew), Encoding.UTF8, 65536);
                string Head = "";
                ...
                sw.Close();
            LogsService.Logar("ConsultaEstoquePreACD_NEW2.aspx", $"Finalizou a montagem do arquivo as {DateTime.Now.ToString()}");
